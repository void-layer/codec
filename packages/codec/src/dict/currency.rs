...
use crate::dict::data::v1_currency::V1_CURRENCY_DICT_ENTRIES;

pub(crate) static CURRENCY_DICT: &[(u8, &str)] = V1_CURRENCY_DICT_ENTRIES;
...
mod tests {
    ...
    fn hash_currency_dict() -> String {
        let mut buf = Vec::new();
        for (code, sym) in crate::dict::data::v1_currency::V1_CURRENCY_DICT_ENTRIES {
            buf.push(*code);
    ...
    fn currency_dict_matches_v1_entries() {
        assert_eq!(
            CURRENCY_DICT.len(),
            crate::dict::data::v1_currency::V1_CURRENCY_DICT_ENTRIES.len(),
